...
export default function Profile() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_PHOTO_SIZE) {
      setUploadMsgType('error');
      setUploadMsg('Image must be 5MB or smaller.');
      if (fileInputRef.current) fileInputRef.current.value = '';
      return;
    }
    ...
    try {
      ...
      setUploadMsgType('error');
      setUploadMsg(err?.response?.data?.message || 'Photo upload failed. Try again.');
      setPreviewUrl(null);
    } finally {
      setUploading(false);
      if (fileInputRef.current) fileInputRef.current.value = '';
  ...
  const displayPhoto = previewUrl || formData.photoUrl || user.photoUrl || DEFAULT_AVATAR;
        ...
        <div className="bg-white border border-slate-100 rounded-2xl shadow-xl p-6 sm:p-8">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <button
                    ...
                    disabled={uploading}
                    className="flex items-center gap-2 px-4 py-2.5 bg-violet-50 hover:bg-violet-100 disabled:bg-slate-100 text-violet-700 disabled:text-slate-400 text-sm font-semibold rounded-xl border border-violet-200 disabled:border-slate-200 transition cursor-pointer disabled:cursor-not-allowed"
                  >
                    {uploading ? (
                      <>
              ...
              {uploadMsg && (
                <div className={`mt-2 p-2.5 text-xs font-medium rounded-xl ${
